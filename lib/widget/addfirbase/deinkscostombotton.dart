...
Widget buildAddProductButtondrink(
        ...
        onPressed: () {
          if (_formKey.currentState?.saveAndValidate() ?? false) {
            ...
            DatabaseReference newProductRef =
                databaseReference.child('beverages').push();
            newProductRef.set(formData).then((_) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  backgroundColor: Colors.green,
                  content: Text('تم إضافة المشروب بنجاح!'),
                ),
              );
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text('حدث خطأ أثناء إضافة المشروب: $error'),
                ),
              );
            });
          }
